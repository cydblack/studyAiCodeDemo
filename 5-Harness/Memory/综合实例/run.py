"""综合案例：把文件记忆接入 Workspace，再执行新任务。

Workspace：它把文件产物导入 Workspace，
...
from memory_workspace import FileMemoryWorkspace
    ...
    for row in consolidated_sessions:
        await workspace.ingest(
            ...
            source={
                ...
                "file": "consolidated_sessions.jsonl",
            },
...
async def check_rule_compliance(
    stable_rules: list[str], draft: dict[str, Any]
) -> list[str]:
    if not stable_rules:
        return []
    ...
    async def request() -> dict[str, Any]:
        ...
        if not isinstance(result, dict) or not isinstance(result.get("规则遵守"), list):
            raise RuntimeError(
                "transient_model_parse_failed: compliance check returned invalid structure"
            )
        return result
...
def apply_memory_guards(stable_rules: list[str], draft: dict[str, Any]) -> None:
    ...
    if (
        "路线检查" in joined_rules or "检查结论" in joined_rules
    ) and "路线检查结论" not in joined_notes:
        notes.append(
            "路线检查结论：迪士尼作为远距离大项已独占一整天，未与市区景点混排；"
            "如需生产级结论，应调用真实 route_checker 后替换本条模拟检查。"
        )
    if (
        "午睡" in joined_rules or "午休" in joined_rules
    ) and "14:00-16:00" not in joined_notes:
        notes.append("午休安排：14:00-16:00 预留回酒店或童车午睡窗口，不安排排队项目。")
    draft["输出附带"] = notes


async def draft_itinerary(
    task: str, task_brief: dict[str, Any] | None
) -> dict[str, Any]:
    ...
    async def request() -> dict[str, Any]:
        ...
        if (
            ...
            or "行程骨架" not in result
            or "输出附带" not in result
        ):
            raise RuntimeError(
                "transient_model_parse_failed: itinerary draft returned invalid structure"
            )
        return result
...
async def main() -> None:
    ...
    workspace = FileMemoryWorkspace(DEMO_ROOT / "workspace")
    ...
    print_json(
        {
            "workspace_integration": {
                ...
                "workspace_root": str(
                    (DEMO_ROOT / "workspace").relative_to(LESSON_DIR)
                ),
            },
