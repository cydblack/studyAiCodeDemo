...
```bash
python Step1_读取会话并压缩存入文件.py         # 会话压缩(Map)
python Step2_升格把候选记忆升格成长期记忆.py    # 升格长期记忆(Reduce)
python Step3_新会话召回.py                    # 新会话召回
python Step4_过程记忆召回.py                  # 过程记忆召回
python Step5_提取能力记忆.py                  # 能力/方法记忆
```
