...
def evaluate_wealth_advisor():
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(
            ...
            "case1-hybrid_wealth_advisor_langgraph_langsmith.py",
        )
        ...
        print(f"\n[WARN] 无法导入投顾助手模块: {e}")
        print("  请确保 case1-hybrid_wealth_advisor_langgraph_langsmith.py 文件存在")
        return
...
- 如果没有包含任何关键词，给出 0.0 分"""
    ...
    try:
        ...
        client.create_dataset(
            dataset_name=dataset_name, description="投顾助手 OpenEvals 评估数据集"
        )
            ...
            if results_list:
                ...
                for key in predefined_keys:
                    if key in scores_summary:
                        ...
                        print(
                            f"    {key:20s}: 平均分数 {avg_score:.3f} ({len(scores)} 个样本)"
                        )
