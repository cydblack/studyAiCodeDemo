...
class CustomerProfile(BaseModel):
    ...
    investment_horizon: Literal["短期", "中期", "长期"] = Field(
        ..., description="投资期限"
    )
...
class InvestmentAnalysisOutput(BaseModel):
    """深度投资分析结果（字段均为简单类型，避免模型输出非法嵌套 JSON）"""

    market_assessment: str = Field(..., description="市场评估，自然语言文本")
    portfolio_analysis: str = Field(..., description="投资组合分析，自然语言文本")
    recommendations: List[str] = Field(..., description="投资建议列表，每项为一条建议文本")
    risk_analysis: str = Field(..., description="风险分析，自然语言文本")
    expected_outcomes: str = Field(..., description="预期结果和回报预测，自然语言文本")
...
class WealthAdvisorState(TypedDict):
    ...
    # 处理状态
    query_type: Optional[
        Literal["emergency", "informational", "analytical"]
    ]  # 查询类型
...
def _load_prompts() -> Dict[str, str]:
    """从 prompts 目录加载提示词"""
    prompts_path = Path(__file__).parent / "prompts" / "投顾助手+langsmith_prompts.yaml"
    with open(prompts_path, encoding="utf-8") as f:
        return yaml.safe_load(f)


_PROMPTS = _load_prompts()
ASSESSMENT_PROMPT = _PROMPTS["ASSESSMENT"]
REACTIVE_PROMPT = _PROMPTS["REACTIVE"]
DATA_COLLECTION_PROMPT = _PROMPTS["DATA_COLLECTION"]
ANALYSIS_PROMPT = _PROMPTS["ANALYSIS"]
RECOMMENDATION_PROMPT = _PROMPTS["RECOMMENDATION"]
REACTIVE_AGENT_PROMPT = _PROMPTS["REACTIVE_AGENT"]
REACTIVE_FINAL_PROMPT = _PROMPTS["REACTIVE_FINAL"]
...
def reactive_processing(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        ...
        if "[CALL_TOOL:上证指数查询]" in first_response:
            ...
            final_prompt = ChatPromptTemplate.from_template(REACTIVE_FINAL_PROMPT)
            final_chain = final_prompt | llm | StrOutputParser()
            final_response = final_chain.invoke(
                {"input": user_query, "tool_result": tool_result}
            )
        else:
            final_response = first_response.replace(
                "[CALL_TOOL:上证指数查询]", ""
            ).strip()
    ...
    try:
        ...
        input_data = {
            ...
            "customer_profile": json.dumps(
                state.get("customer_profile", {}), ensure_ascii=False, indent=2
            ),
        }
...
def analyze_data(state: WealthAdvisorState) -> WealthAdvisorState:
    ...
    try:
        ...
        # 用 Pydantic schema 约束输出结构，避免模型产出非法嵌套 JSON
        parser = JsonOutputParser(pydantic_object=InvestmentAnalysisOutput)
        ...
        input_data = {
            ...
            "market_data": json.dumps(
                state.get("market_data", {}), ensure_ascii=False, indent=2
            ),
    ...
    try:
        ...
        input_data = {
            ...
            "analysis_results": json.dumps(
                state.get("analysis_results", {}), ensure_ascii=False, indent=2
            ),
        }
...
def create_wealth_advisor_workflow() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        "assess",
        lambda x: (
            "reactive" if x.get("processing_mode") == "reactive" else "collect_data"
        ),
...
def run_wealth_advisor(
    user_query: str, customer_id: str = "customer1"
) -> Dict[str, Any]:
    """运行财富顾问智能体并返回结果"""
        ...
        if LANGSMITH_ENABLED:
            ...
            config = RunnableConfig(
                ...
                metadata={
                    ...
                    "investment_horizon": customer_profile.get(
