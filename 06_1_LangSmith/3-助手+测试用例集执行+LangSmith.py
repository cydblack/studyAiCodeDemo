...
# 导入主智能体模块（文件名以数字开头，普通 import 不可用，需 importlib）
import importlib.util
import sys

# 基于本脚本所在目录定位，避免从仓库根目录运行时找不到文件
...
REACTIVE_TEST_CASES = [
    ...
    {
        "inputs": {"user_query": "请解释一下什么是ETF？", "customer_id": "customer1"},
        "outputs": {
            "processing_mode": "reactive",
...
EDGE_CASE_TEST_CASES = [
    {
        "inputs": {"user_query": "", "customer_id": "customer1"},  # 空查询
        "outputs": {"should_handle_error": True},
    },
    {
        "inputs": {
    ...
    try:
        ...
        if isinstance(example, dict):
            if "outputs" in example:
                return (
                    example["outputs"] if isinstance(example["outputs"], dict) else {}
                )
        # 尝试直接访问属性
...
def run_evaluation(
    ...
    if not experiment_name:
        experiment_name = (
            f"wealth-advisor-eval-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
        )
    ...
    # evaluate() 传入的是 example.inputs（dict），不是整个 Example
    def test_function(inputs: dict) -> Dict[str, Any]:
        """测试函数：按数据集 inputs 跑智能体并返回结果"""
        try:
            user_query = inputs.get("user_query") or ""
            customer_id = inputs.get("customer_id", "customer1")

            # 边界用例：空查询直接返回错误结果，不调用 Agent
...
if __name__ == "__main__":
    ...
    # 固定的数据集名称
    dataset_name = "wealth-advisor-test-dataset0804"
