...
# Prompt YAML 文件路径
PROMPTS_FILE = Path(__file__).parent / "prompts" / "混合式_prompts.yaml"
...
REACTIVE_SYSTEM_PROMPT = _PROMPTS["REACTIVE_SYSTEM"]
...
def query_portfolio_allocation(asset_type: str) -> str:
    ...
    """
    # 模拟数据
    allocations = {"股票": "40%", "债券": "30%", "现金": "10%", "另类投资": "20%"}
...
def assess_query(state: WealthAdvisorState) -> Dict[str, Any]:
    ...
    print(
        f"[DEBUG] 分支判断: processing_mode={processing_mode}, query_type={query_type}"
    )
...
def reactive_agent(state: WealthAdvisorState) -> Dict[str, Any]:
    ...
    if not messages:
        content = REACTIVE_SYSTEM_PROMPT.format(
...
def extract_reactive_response(state: WealthAdvisorState) -> Dict[str, Any]:
    ...
    messages = state.get("messages", [])
    print("[DEBUG] 当前 messages 快照:")
    print(
        json.dumps(
            [
                _serialize_message(m) if isinstance(m, BaseMessage) else m
                for m in messages
            ],
            ensure_ascii=False,
            indent=2,
    ...
    for msg in reversed(messages):
        if isinstance(msg, AIMessage) and msg.content:
            print(f"[DEBUG] 提取 final_response 长度: {len(msg.content)}")
...
def serialize_state_for_log(state: Dict[str, Any]) -> Dict[str, Any]:
    """导出可记录的状态快照（messages 转为普通结构）"""
    snapshot = dict(state)
    messages = snapshot.get("messages") or []
    snapshot["messages"] = [
        _serialize_message(m) if isinstance(m, BaseMessage) else m for m in messages
    ]
    return snapshot
...
def collect_data(state: WealthAdvisorState) -> Dict[str, Any]:
    ...
    print("[DEBUG] collect_data LLM输出:")
...
def analyze_data(state: WealthAdvisorState) -> Dict[str, Any]:
    ...
    print("[DEBUG] analyze_data LLM输出:")
    print(json.dumps(result, ensure_ascii=False, indent=2))
...
def generate_recommendations(state: WealthAdvisorState) -> Dict[str, Any]:
    ...
    print("[DEBUG] generate_recommendations 文本长度:", len(result or ""))
...
def create_wealth_advisor_workflow() -> StateGraph:
    ...
    workflow.add_conditional_edges(
        ...
        lambda x: (
            "reactive_agent"
            if x.get("processing_mode") == "reactive"
            else "collect_data"
        ),
        {"reactive_agent": "reactive_agent", "collect_data": "collect_data"},
    )
    ...
    workflow.add_conditional_edges(
        ...
        {"tools": "tools", "end": "extract_response"},
    )
...
def run_wealth_advisor(
    ...
    print("[DEBUG] 最终状态 JSON：")
    print(
        json.dumps(
            serialize_state_for_log(result),
            ensure_ascii=False,
            indent=2,
            default=str,
        )
