...
def answer_question(query: str) -> str:
    ...
    if best_score < 0.2 or best_rule is None:
        return (
            "在知识库中未找到与该问题直接相关的信息。请尝试使用关键词搜索或类别查询。"
        )
...
"""
    ...
    # 调用 langchain.agents.create_agent，勿与本函数同名以免遮蔽
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print()
        # Message 对象不可直接 json.dumps，先转成可序列化结构
        response_json = {"messages": [msg.model_dump() for msg in response["messages"]]}
        print("response:", json.dumps(response_json, ensure_ascii=False, indent=2))
        print()
