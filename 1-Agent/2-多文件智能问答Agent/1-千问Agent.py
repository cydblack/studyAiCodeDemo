...
2. 带图形界面
3. 使用多文件智能问答Agent
4. 数据使用了Qwen自带的BM25模型进行召回, 属于关键字召回，不需要单独安装向量库,速度快，准确性高。
"""
...
llm_cfg = {
    ...
    "api_key": DASHSCOPE_API_KEY,  # 从环境变量获取API Key
...
def get_doc_files():
    """获取 docs 文件夹下的所有文件"""
    file_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "docs")
    files = []
