...
def _in_persist_dir(persist_dir: str, fn):
    """FAISS 底层用 C fopen，Windows 打不开含中文的绝对路径；先切到目录内再用相对路径。"""
...
def load_documents_and_create_index(
    file_dir: str = os.path.join(_BASE_DIR, "docs"),
    persist_dir: str = os.path.join(_BASE_DIR, "langchain_storage"),
):
    ...
    _in_persist_dir(persist_dir, lambda: vector_store.save_local("."))
...
def main():
    ...
    # 配置 LLM
    llm = ChatTongyi(
        model_name="deepseek-v4-flash", dashscope_api_key=DASHSCOPE_API_KEY
    )
