...
def recommendations(name, cosine_similarities, indices):
    recommended_hotels = []
    # 找到想要查询酒店名称的 idx
    ...
    for i in top_10_indexes:
        recommended_hotels.append(indices.iloc[i])
    return recommended_hotels


def main():
    # 打印数据
    print(df.head(2))  # 打印 df 的前几行，默认是 5 行，用来看一眼数据长什么样。
    ...
    print(
        recommendations(
            "Hilton Seattle Airport & Conference Center", cosine_similarities, indices
        )
    )

    print(
        recommendations(
            "The Bacon Mansion Bed and Breakfast", cosine_similarities, indices
        )
    )
