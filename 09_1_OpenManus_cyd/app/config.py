...
class Config:
    ...
    def _load_initial_config(self):
        ...
        if daytona_config:
            # 优先使用配置文件中的 daytona_api_key，为空则从环境变量 DAYTONA_API_KEY 读取
            if not daytona_config.get("daytona_api_key"):
                daytona_config["daytona_api_key"] = os.getenv("DAYTONA_API_KEY")
            daytona_settings = DaytonaSettings(**daytona_config)
        elif os.getenv("DAYTONA_API_KEY"):
            # 未配置 [daytona] 段时，仅环境变量也可启用 Daytona
